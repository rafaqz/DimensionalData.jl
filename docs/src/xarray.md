...
automatically convert it. Otherwise they will be passed as opaque Julia objects,
though a [`DimArray`](@ref) will still support the array interface in Python so
it can be used with e.g. numpy.
...
- Python stores arrays in row-major order whereas Julia stores them in
  ...
  Julia and Python.
- Only the metadata of the array/stack itself is converted to `attrs`. Any
  metadata attached to lookups (e.g. units) is not passed to the coordinates
  when converting to Python, and likewise coordinate `attrs` are not converted
  to lookup metadata when converting to Julia.
