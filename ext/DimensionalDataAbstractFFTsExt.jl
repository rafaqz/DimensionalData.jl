...
function reinterpret_into_datatype(T::Type{<:Number}, vec)
    reinterpret_into_datatype(one(T), vec)
end,
...
function _step(x::DD.Dimensions.Dimension{<:AbstractFFTs.Frequencies})
    step(parent(lookup(x)))
end,
function _step(x::DimensionalData.Dimensions.Dimension{<:DimensionalData.Dimensions.Lookups.Sampled{<:Any, <:Frequencies{<:Any}}})
    step(parent(lookup(x)))
end
...
function _ifftfreq(dd_lookup)
    ...
    DD.basetypeof(dd_lookup)(range(zero(dx), step = dx, length = len))
end
...
"""
@noinline function get_freqs(plan::P, dd::DD.AbstractDimArray{<:Any, N}) where {P<:AbstractFFTs.Plan, N}
...
"""
function get_scale_factor(plan::P, dd::DD.AbstractDimArray{T, N}) where {T,P<:AbstractFFTs.Plan, N}
    fft_dims = AbstractFFTs.fftdims(plan)
    is_fft_dims = ntuple(i -> i in fft_dims, N)
...
function LinearAlgebra.mul!(dd_out::DD.AbstractDimArray{Tout, N}, plan::DDPlan{<:Any, Tp}, dd_in::DD.AbstractDimArray{Tin, N}) where {N, Tin, Tout, Tp}
    ...
    LinearAlgebra.mul!(reinterpret_into_datatype(Tout, parent(dd_out)), plan.p, reinterpret_into_datatype(Tin, parent(input_to_plan)))
    if !is_inverse_transform(plan)
...
function AbstractFFTs.plan_ifft(dd::AbstractDimArray{T, N}, dd_dims = ntuple(identity, N); kwargs...) where {N, T}
    ...
    s = get_scale_factor(p, dd)
    unitless_s = s / oneunit(s)
    DDPlan{typeof(s), Inverse, ComplexFFT}(AbstractFFTs.ScaledPlan(p, unitless_s), similar(dd))
end
...
function AbstractFFTs.plan_irfft(dd::AbstractDimArray{T, N}, len::Integer, dd_dims = ntuple(identity, N); kwargs...) where {N, T}
    dims = DD.dimnum(dd, dd_dims)
    p = AbstractFFTs.plan_brfft(reinterpret_into_datatype(T, parent(dd)), len, dims; kwargs...)
    s = get_scale_factor(p, dd)
    unitless_s = s / oneunit(s)
    DDPlan{typeof(s), Inverse, RealFFT}(AbstractFFTs.ScaledPlan(p, unitless_s), similar(dd))
end
