...
SnoopPrecompile.@precompile_all_calls begin
    for f in (zeros, ones, falses, trues, rand)
        ...
        show(devnull, MIME"text/plain"(), da1)
        show(devnull, MIME"text/plain"(), da2)
        show(devnull, MIME"text/plain"(), da3)
        show(devnull, MIME"text/plain"(), da4)
        st1 = DimStack(da1, da2)
        show(devnull, MIME"text/plain"(), st1)
        st2 = DimStack(da1, da2, da3)
        show(devnull, MIME"text/plain"(), st2)
        st3 = DimStack(da1, da2, da3, da4)
        show(devnull, MIME"text/plain"(), st3)
        mst1 = merge(st1, st2)
        mst2 = merge(st1, st3)
    end
end
