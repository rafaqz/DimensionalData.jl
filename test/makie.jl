...
@testset begin 
    ...
    function test_1d_plot(plot_function, _dd)
        ...
        if !(eltype(x) <: AbstractChar) # Test if change in x axis of observable works
    ...
    for dd_i in (dd_mat_cat, dd_mat_num, dd_mat_mis) 
...
@testset begin
    ...
    for dd_i in (dd_3d_uni, dd_3d_rgb) # Broken due to limitations in Makie
        @test test_3d_plot(volume, dd_3d_uni; x = X, y = Y, z = Z) broken = true 
        @test test_3d_plot(volumeslice, dd_3d_uni; x = X, y = Y, z = Z) broken = true 
    end
end
