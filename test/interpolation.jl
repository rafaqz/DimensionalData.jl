...
y_vec = [14.7, 11.51, 10.41, 14.95, 12.24, 11.22]

dy_vec = [-0.047, -0.058, 0.054, 0.012, -0.068, 0.0011]

ddy_vec = [0.0, -0.00033, 0.0051, -0.0067, 0.0029, 0.0]

y_dimvec = DimArray(y_vec, X(x_vec))
...
@testset "Reproduces Data" begin
    @testset "`$Itp`" for Itp in reproducers
        itp = if Itp == BSplineInterpolation
            BSplineInterpolation(y_dimvec, 3, :ArcLen, :Average)
        elseif Itp == CubicHermiteSpline
            CubicHermiteSpline(dy_vec, y_dimvec)
        elseif Itp == QuinticHermiteSpline
            QuinticHermiteSpline(ddy_vec, dy_vec, y_dimvec)
        else
            Itp(y_dimvec)
        end

        if Itp in [CubicSpline, QuinticHermiteSpline]
    ...
    @testset "Backward `QuadraticInterpolation`" begin
        itp = QuadraticInterpolation(y_dimvec, :Backward)
        @testset "$x" for x in x_vec
            @test isequal(
                y_dimvec[X = At(x)],
                itp(x)
    ...
    @testset "Right `ConstantInterpolation`" begin
        itp = ConstantInterpolation(y_dimvec; dir = :Right)
        @testset "$x" for x in x_vec
            @test isequal(
                y_dimvec[X = At(x)],
                itp(x)
...
@testset "Preserves Interpolation" begin
    @testset "$Itp" for Itp in interpolators
        (; itp_raw, itp_dim) = if Itp == BSplineInterpolation
            itp_raw = BSplineInterpolation(y_vec, x_vec, 3, :ArcLen, :Average)
            itp_dim = BSplineInterpolation(y_dimvec, 3, :ArcLen, :Average)
            (; itp_raw, itp_dim)
        elseif Itp == CubicHermiteSpline
            itp_raw = CubicHermiteSpline(dy_vec, y_vec, x_vec)
            itp_dim = CubicHermiteSpline(dy_vec, y_dimvec)
            (; itp_raw, itp_dim)
        elseif Itp == QuinticHermiteSpline
            itp_raw = QuinticHermiteSpline(ddy_vec, dy_vec, y_vec, x_vec)
            itp_dim = QuinticHermiteSpline(ddy_vec, dy_vec, y_dimvec)
            (; itp_raw, itp_dim)
        else
            itp_raw = Itp(y_vec, x_vec)
            itp_dim = Itp(y_dimvec)
            (; itp_raw, itp_dim)
        end
    ...
    @testset "Backward `QuadraticInterpolation`" begin
        itp_raw = QuadraticInterpolation(y_vec, x_vec, :Backward)
        itp_dim = QuadraticInterpolation(y_dimvec, :Backward)
        @testset "$x" for x in x_vec
            @test isequal(
                itp_raw(x),
                itp_dim(x)
    ...
    @testset "Right `ConstantInterpolation`" begin
        itp_raw = ConstantInterpolation(y_vec, x_vec; dir = :Right)
        itp_dim = ConstantInterpolation(y_dimvec; dir = :Right)
        @testset "$x" for x in x_vec
            @test isequal(
                itp_raw(x),
                itp_dim(x)
        ...
        @testset "$Itp" for Itp in interpolators
            ...
            (; itp_raw, itp_dim) = if Itp == BSplineInterpolation
                itp_raw = BSplineInterpolation(
                    y_vec, x_vec, 3, :ArcLen, :Average;
                    extrapolation = Xtp
                )
                itp_dim = BSplineInterpolation(
                    y_dimvec, 3, :ArcLen, :Average;
                    extrapolation = Xtp
                )
                (; itp_raw, itp_dim)
            ...
            elseif Itp == QuinticHermiteSpline
                itp_raw = QuinticHermiteSpline(
                    ddy_vec, dy_vec, y_vec, x_vec;
                    extrapolation = Xtp
                )
                itp_dim = QuinticHermiteSpline(
                    ddy_vec, dy_vec, y_dimvec;
                    extrapolation = Xtp
                )
                (; itp_raw, itp_dim)
