...
@testset "getindex returns DimensionArray slices with the right dimensions" begin
    ...
    @test dims(a) == (X(LinRange(143.0, 145.0, 2); grid=PointGrid()),)
    @test refdims(a) == (Y(-38.0; grid=PointGrid()),)
    ...
    @test dims(a) == (Y(LinRange(-38.0, -36.0, 2); grid=PointGrid()),)
    @test refdims(a) == (X(143.0; grid=PointGrid()),)
    ...
    @test dims(a) == (X(LinRange(143.0, 145.0, 2); grid=PointGrid()), 
                      Y(LinRange(-38.0, -36.0, 2); grid=PointGrid()))
    @test refdims(a) == ()
...
@testset "view returns DimensionArray containing views" begin
    ...
    @test dims(v) == ()
    @test refdims(v) == (X(143.0; grid=PointGrid()), 
                         Y(-38.0; grid=PointGrid()))
    @test bounds(v) == ()
    ...
    @test dims(v) == (X(LinRange(143.0, 145.0, 2); grid=PointGrid()),)
    @test refdims(v) == (Y(-38.0; grid=PointGrid()),)
    ...
    @test dims(v) == (X(LinRange(143.0, 143.0, 1); grid=PointGrid()), 
                      Y(LinRange(-38, -36, 2); grid=PointGrid()))
    ...
    @test dims(v) == (Y(LinRange(-38.0, -36.0, 2); grid=PointGrid()),)
    @test refdims(v) == (X(143.0; grid=PointGrid()),)
...
@testset "broadcast" begin
    ...
    @test dims(da2) == (Y(LinRange(10, 20, 5); grid=PointGrid()), 
                        Ti(10:11; grid=PointGrid()), 
                        X(1:4; grid=PointGrid()))
