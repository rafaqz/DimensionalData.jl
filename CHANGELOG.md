...
- `unsafe_set` was split out from `set` in ([#926])  - `set` now fixes everything to return a
  correct object, and `unsage_set` does not. Use `set` directly, and
  `unsafe_set` in packages or if you really know what youre doing.
